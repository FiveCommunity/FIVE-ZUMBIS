		...
		if LocalPlayer["state"]["Active"] then
			...
			if GetEntityHealth(Ped) > 100 then
				if Hunger <= 10 and HungerTimer <= GetGameTimer() then
					...
					TriggerEvent("Notify","Alimentação","Sofrendo com a <b>fome</b>.","fome",8000)
				end

				if Thirst <= 10 and ThirstTimer <= GetGameTimer() then
					ApplyDamageToPed(Ped,1,false)
					ThirstTimer = GetGameTimer() + 60000
					TriggerEvent("Notify","Hidratação","Sofrendo com a <b>sede</b>.","sede",8000)
				end

				if Stress ~= 999 and Stress >= 15 and StressTimer <= GetGameTimer() then
					ApplyDamageToPed(Ped,1,false)
					StressTimer = GetGameTimer() + 30000
					TriggerEvent("Notify", "Tosse", "Sofrendo com a <b>Contaminação</b>.", "radiacao", 8000)
