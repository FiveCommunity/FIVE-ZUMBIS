	...
	if Passport and Consult and Consult ~= "0" then
		PerformHttpRequest("https://discordlookup.mesalytic.moe/v1/user/"..Consult,function(Return,Response)
			if Return == 200 then
				...
				if Result and Result.avatar.link then
...
function vRP.GetWeight(Passport, Ignore)
    ...
    if Datatable then
        ...
        for Permission, _ in pairs(UserGroups) do
            if vRP.Permissions(Permission,"Premium") > 0 then
                Premium = true
                break
            end
        end
...
function vRP.GiveBank(Passport,Amount,Notify)
	...
	if Characters[source] then
		...
		if Notify then
			TriggerClientEvent("NotifyItem",source,{ "+","zcoin",Amount,ItemName("zcoin"),ItemRarity("zcoin") })
		end
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
...
function vRP.PaymentFull(Passport,Amount,Notify)
	...
	if Amount > 0 then
		return vRP.TakeItem(Passport,"zcoin",Amount,Notify) or vRP.PaymentBank(Passport,Amount,Notify)
	end
...
function vRP.WithdrawCash(Passport,Amount)
	...
	if Amount > 0 and Characters[source] and Characters[source].Bank >= Amount then
		vRP.GenerateItem(Passport,"zcoin",Amount,true)
		vRP.RemoveBank(Passport,Amount,source)
	...
	if Datatable then
		if not Datatable["Rolepass"] or (Datatable["Rolepass"] and Datatable["Rolepass"]["Clean"] < os.time()) then
			Datatable["Rolepass"] = {
				...
				["Premium"] = 0,
				["Active"] = false,
				["Clean"] = BATTLEPASS_START + 2592000
			}
		end

		return Datatable["Rolepass"],BATTLEPASS_START
	end
...
function vRP.BattlepassBuy(Passport)
	...
	if Datatable then
		...
		Datatable["Rolepass"]["Active"] = BATTLEPASS_START + 2592000
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- BATTLEPASSPAYMENT
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.BattlepassPayment(Passport,Amount,Mode)
	local Datatable = vRP.Datatable(Passport)
...
-- BATTLEPASSPOINTS
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.BattlepassPoints(Passport,Amount,Notify)
	local Datatable = vRP.Datatable(Passport)
	if Datatable then
		if not Datatable["Rolepass"] or (Datatable["Rolepass"] and Datatable["Rolepass"]["Clean"] < os.time()) then
			Datatable["Rolepass"] = {
